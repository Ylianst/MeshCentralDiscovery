...
namespace MeshCentralDiscovery
{
    ...
    public sealed class MeshDiscovery
    {
        ...
        private byte[] discoverykey = null;
        private RandomNumberGenerator discoveryRandom = null;

        public MeshDiscovery(string discoverySecret)
        {
            // Setup the discovery key if needed.
            if ((discoverySecret != null) && (discoverySecret.Length > 0))
            {
                SHA384 shaM = new SHA384Managed();
                byte[] hash = shaM.ComputeHash(UTF8Encoding.UTF8.GetBytes(discoverySecret));
                discoverykey = new byte[32];
                Array.Copy(hash, 0, discoverykey, 0, 32);
                discoveryRandom = RandomNumberGenerator.Create();
            }
                ...
                try
                {
                    ...
                    if (buffer != null)
                    {
                        ...
                        if ((values.Length >= 3) && (values[0] == "MeshCentral2"))
                        {
                            ...
                            if (values.Length == 3)
                            {
                                byte[] EncryptedPingPacket = encryptPacket(PingPacket);
                                session.Send(EncryptedPingPacket, EncryptedPingPacket.Length, ep); session.Send(EncryptedPingPacket, EncryptedPingPacket.Length, ep);
                            }
                        }
                    }
                }
        ...
        */
        ...
        private void MulticastPing(UdpClient session)
        {
            try
            {
                ...
                if (session.Client.AddressFamily == AddressFamily.InterNetwork)
                {
                    ...
                    byte[] EncryptedPingPacket = encryptPacket(PingPacket);
                    session.Send(EncryptedPingPacket, EncryptedPingPacket.Length, MulticastV4EndPoint);
                }
                else if (session.Client.AddressFamily == AddressFamily.InterNetworkV6 && address.ScopeId != 0)
                {
                    ...
                    byte[] EncryptedPingPacket = encryptPacket(PingPacket);
                    session.Send(EncryptedPingPacket, EncryptedPingPacket.Length, MulticastV6EndPoint);
                }
            }
        ...
        public void UnicastData(byte[] buffer, IPAddress local, IPEndPoint remote)
        {
            ...
            try
            {
                byte[] encryptedPacket = encryptPacket(buffer);
                session.Send(encryptedPacket, encryptedPacket.Length, remote);
            }
            catch (SocketException) { }
        }
        ...
        byte[] encryptPacket(byte[] packet)
        {
            if (discoverykey == null) { return packet; }

            // Setup the Initialization Vector
            byte[] iv = new byte[16];
            discoveryRandom.GetBytes(iv);
            ...
            // Encrypt and append IV in front
            byte[] r = rijndaelManaged.CreateEncryptor().TransformFinalBlock(packet, 0, packet.Length);
            byte[] total = new byte[16 + r.Length];
            Array.Copy(iv, 0, total, 0, 16);
            Array.Copy(r, 0, total, 16, r.Length);
            return total;
        }

        byte[] decryptPacket(byte[] packet)
        {
            if (discoverykey == null) { return packet; }
            try
            {
                ...
                return rijndaelManaged.CreateDecryptor().TransformFinalBlock(packet, 16, packet.Length - 16);
